...
class SimpleCNNModelV2(nn.Module):
    def __init__(self, input_c, input_h, input_w, kernel_sizes, extra_padding,
                 hidden_c_sizes=[10,10], final_c=None, activation=None,cuda=False):
        nn.Module.__init__(self)
        self.use_cuda = cuda
        if activation is None:
            activation = nn.ReLU
...
class OtherCNN(SimpleCNNModel):
  def __init__(self,cuda):
    SimpleCNNModel.__init__(self,input_c=1,input_h=28,input_w=28,
                          kernel_sizes=[3,3],extra_padding=[0,1],
                          activation=nn.ReLU,cuda=cuda)

  def forward(self,data):
    data = data.view(data.shape[0],1,28,28)
    return SimpleCNNModel.forward(self,data)


class OtherCNNV2(SimpleCNNModelV2):
  def __init__(self,cuda):
    SimpleCNNModelV2.__init__(self,input_c=1,input_h=28,input_w=28,
                          kernel_sizes=[3,3],extra_padding=[0,1],
                          activation=nn.ReLU,cuda=cuda)

  def forward(self,data):
    data = data.view(data.shape[0],1,28,28)
    return SimpleCNNModelV2.forward(self,data)

class OtherCNNV3(SimpleCNNModelV3):
  def __init__(self,cuda):
    SimpleCNNModelV3.__init__(self,input_c=1,input_h=28,input_w=28,
                          kernel_sizes=[3,3],extra_padding=[0,1],
                          activation=nn.ReLU,cuda=cuda)

  def forward(self,data):
    data = data.view(data.shape[0],1,28,28)
    return SimpleCNNModelV3.forward(self,data)
